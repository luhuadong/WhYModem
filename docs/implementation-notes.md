...
每次点击“发送”或“接收”开始新一轮传输时，`FileTransmitter` 和 `FileReceiver` 都会先停止读写定时器、关闭串口、关闭文件句柄，并重新创建协议对象。串口重新打开后会调用 `QSerialPort::clear(QSerialPort::AllDirections)` 清理本端输入/输出缓冲。这样可以确保上一轮取消、超时或失败后不会留下协议阶段、文件读写位置、串口缓存或定时器状态，避免出现第一次失败后第二次“假成功”或复用 EOF 文件位置的问题。

超时和中止路径也会关闭对应文件句柄，确保下一轮传输可以重新从文件开头开始。

### 2.1 配置文件

WhYModem 使用 Qt 的 `QSettings` 以 ini 格式保存应用配置。程序启动时会自动创建默认配置文件。GUI 左侧栏底部的设置按钮可以直接修改发送延迟；点击“保存”后会写入 ini，并同步应用到当前发送器，因此正在进行的 YMODEM 发送会从后续数据包开始使用新参数。
...
注意：`QSerialPort::clear()` 只能清理 WhYModem 当前打开这一端的缓冲。如果旧版本、上一轮失败或协议切换前的测试已经把 `C`、`NAK`、`CAN`、`EOT` 等字节写到了命令行工具那一端的 pty 队列中，WhYModem 无法从本端清掉这些字节。使用 `socat`/pty 做协议切换、失败重试或取消后重试时，建议关闭 WhYModem 串口并重启 `socat` 虚拟串口对，确保两端队列为空。

## 4. 接收等待与超时策略

XMODEM 和 YMODEM 接收在“尚未收到第一个有效数据包”时采用持续等待策略。GUI 端会周期性发送握手字符：XMODEM 持续发送 `C` 请求 CRC 模式，YMODEM 周期性发送 `C` 等待 block0。此阶段不自动判定失败，由用户点击“取消”退出等待。

XMODEM 接收等待阶段不要在 `C` 和 `NAK` 之间自动切换。真实串口或 `socat` pty 都可能缓冲早先发送的握手字节；如果命令行发送端读到旧的 `C` 选择 CRC，而 GUI 接收端已经降级到 checksum，就会出现双方校验模式错配、重复 NAK、表现随机的问题。

一旦已经收到有效首包并进入传输阶段，协议状态机会恢复错误和超时保护：坏包会触发 `NAK` 或错误计数，连续异常、对端取消或传输中断仍会结束传输并向 UI 上报失败。

ZMODEM 使用 qzmodem 适配层，启动和取消行为与 XMODEM/YMODEM 不同，暂不与本策略合并。
...
同样因为没有文件大小字段，WhYModem 在 XMODEM 接收时无法计算真实百分比。UI 会把接收进度条切换为不确定状态，表示正在接收数据；收到 `EOT` 并完成收尾后再恢复为 100%。XMODEM 发送端不同，发送端知道本地文件大小，因此可以显示确定百分比。
...
不要为了掩盖 `socat`/pty 残留字节而放宽 YMODEM 的 CRC 校验。虚拟串口测试中如果出现“文件已经落盘但最后失败”“切换协议后第一次失败、第二次成功”等随机现象，优先重启虚拟串口对或重新打开串口，而不是接受校验不通过的结束 block。

`lrzsz sb` 偶尔打印 `Retry 0: NAK on sector` 并不一定代表传输失败。`NAK` 是协议允许的重传请求，可能来自启动时序、虚拟串口残留或收尾阶段控制字节交错。只要最后显示 `Transfer complete`，并且 `diff` 或 `sha256sum` 验证文件一致，就可以认为本次传输成功。
